...
import numpy as np
from itertools import combinations  # возвращает итератор со всеми возможными комбинациями элементов
# входной последовательности iterable. Каждая комбинация заключена в кортеж с длинной r элементов,
# в которой нет повторяющихся элементов.
...
def to_canonical(system, sign, goal_func, idx):
    ...
    for i in range(len(copy_system)):
        ...
        if copy_sign[i] == '>=':  # если знак >=
            ...
            copy_goal_func.append(0.0)
            copy_sign[i] = '='
    # теперь переменные без ограничения на знак заменяем новыми, в том числе в ф-ии цели
...
def solve_brute_force(A: list, b: list, c: list):
    vectors = get_all_possible_vectors(A, b)  # получаем всевозможные опорные вектора
