...
def make_tableau(A, b, c):
    ...
    xbasic = [A[i] + [b[i]] for i in range(len(A))]
    # построение нижней строки таблицы
    z = c + [0]
    # print(len(A[0]) + 1, len(c) + 1)
    return xbasic + [z]
...
def get_pivot_position(tableau):
    ...
    if all([r == math.inf for r in restrictions]):
        raise Exception("Warning: Linear program is unbounded.")
...
def simplex(c, A, b):
    ...
    while can_optimized(tableau):
        ...
        print("Индексы ведущего элемента: ", pivot_position)
        # выполним поворот
